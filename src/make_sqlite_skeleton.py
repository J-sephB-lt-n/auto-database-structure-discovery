...
def make_sqlite_skeleton(
    col_pairs: list[tuple], output_db_path: str
) -> None:
    ...
    for tbl_name, tbl_cols in tbl_col_ref.items():
        create_tbl_statement = f'CREATE TABLE {tbl_name}( {", ".join([str(c) for c in tbl_cols])}'
        for col1, col2 in col_pairs:
            if col1[0] == tbl_name:
                create_tbl_statement += f" , FOREIGN KEY ({col1[1]}) REFERENCES {col2[0]}({col2[1]})"
